...
async function withProductColumnFallback<T>(fn: () => Promise<T>): Promise<T> {
    try {
        ...
        // Check for missing SQLite columns before retrying after schema repair.
        if (errorString.includes('no such column') || errorString.includes('column not found')) {
...
export async function cancelExpiredOrders(filters: { productId?: string; userId?: string; orderId?: string } = {}) {
    ...
    try {
        // Keep cleanup operations independent so one damaged row does not block the rest.
        const fiveMinutesAgoMs = Date.now() - RESERVATION_TTL_MS;
        // Preselect expired orders before updating them so card reservations can be released reliably.
